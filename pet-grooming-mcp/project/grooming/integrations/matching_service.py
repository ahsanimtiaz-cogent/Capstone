"""Fuzzy matching: pet → service recommendation, price calculation, free-text → service.

Ported from lang-graph/services/matching_service.py with no behavior changes.
"""
import difflib
import logging
import re
...
def format_services_list(services: List[Dict[str, Any]], pet: Dict[str, Any],
    ...
    lines = []
    for svc in services:
        price, _is_fallback, _reason = price_for_service(svc, pet)
