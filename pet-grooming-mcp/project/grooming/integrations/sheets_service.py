...
Ported from lang-graph/services/sheets_service.py with no behavior changes.
"""
import json
import logging
import re
...
class SheetsRepo:
    ...
    def create_lead(self, discord_user_id: str, name: str = "", phone: str = "",
        ...
        row = [lead_id, created_at, source, str(discord_user_id), name, phone, city, "initiated"]
    ...
    def append_pet(self, lead_id: str, pet: Dict[str, Any]) -> str:
        ...
        row = [
            lead_id, pet_id,
            pet.get("name", ""), pet.get("species", "dog"),
            pet.get("breed", ""), pet.get("weight", ""),
            pet.get("age", ""), pet.get("coat", ""),
            pet.get("notes", ""),
        ]
    ...
    def get_brand_config(self, force_refresh: bool = False) -> Dict[str, Any]:
        now = time.time()
        if (not force_refresh and self._brand_cache is not None
